...
class Upvote
{
    ...
    public static function votePost(int $post_id): Response
    {
        ...
        $stmt = $conn->prepare("SELECT COUNT(up.id) count FROM  ". DB_PREFIX . "upvotes up WHERE up.post_id = ? AND up.user_id = ? GROUP BY up.id");
        ...
        if ($result->num_rows > 0) {
            ...
            $stmt = $conn->prepare("DELETE up FROM  ". DB_PREFIX . "upvotes up WHERE up.post_id = ? AND up.user_id = ?");
            $stmt->bind_param("ii", $post_id, $user->id);
            $stmt->execute();
            ...
            $stmt = $conn->prepare("INSERT INTO  ". DB_PREFIX . "upvotes (post_id, user_id) VALUES (?, ?)");
            $stmt->bind_param("ii", $post_id, $user->id);
            $stmt->execute();
    ...
    public static function hasUpvoted(int $post_id): bool
    {
        ...
        $stmt = $conn->prepare("SELECT COUNT(up.id) FROM  ". DB_PREFIX . "upvotes up WHERE up.user_id = ? AND up.post_id = ? GROUP BY up.id");
